...
class MySpider(scrapy.Spider):
    ...
    def parse_dir_contents(self, response):
        ...
        for sel in response.xpath('//html'):
            ...
            # ------- meta description and keywords of page --------
            head = sel.css('head').extract()[0]
            soup = BeautifulSoup(str(head),"html.parser");
            
            for meta in soup.findAll('meta'):
                try:
                    if str(meta['name']).lower() == 'description':
                        dictionary['description'] = meta['content'];
                except:
                    pass

                try:
                    if str(meta['name']).lower() == 'keywords':
                        dictionary['keywords'] = meta['content'];
                except:
                    pass
            ...
            for script in soup(["head","script","style","noscript","footer","link","font","form"]):            #remove tags
...
def main(main_url,main_domain,common):
    try:
        os.remove('xyz.txt')
        os.remove('data.json')
    except OSError:
    ...
    list_of_dict[0]['content'] = removeComments(list_of_dict[0]['content']);

    soup = []
    soup.append(BeautifulSoup(list_of_dict[0]['content'],"html.parser"))
    ...
    print list_of_dict[0]['description'];
    return [list_of_dict[0]['content'],list_of_dict[0]['title'],list_of_dict[0]['old_url'],list_of_dict[0]['images'],list_of_dict[0]['description'],list_of_dict[0]['keywords']]

        
#main();
#main(["http://www.voiceofstudents.org/"],"voiceofstudents.org","");
