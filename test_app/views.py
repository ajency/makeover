...
class TestView(APIView):
    ...
    def post(self, request, format=None):
        ...
        try:
            while True:
                print data["email"]
                data["email"] = ''.join(random.choice(data["email"]) for i in range(50))
                print data["email"]
                user = User.objects.get(username=str(data["email"]))
        except:
            try:
                user = User.objects.create_user(str(data["email"]), "password", str(email))
                user.save()
            except:
                return Response(
                        'Database Error.',
